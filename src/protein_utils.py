...
def build_protein_encoder(
    ...
    try:
        ...
        if "size mismatch" in msg:
            raise RuntimeError(
                "Failed to load protein encoder weights due to a size mismatch. "
...
def process_protein_sequences(
    ...
    *,
    ...
    # full VEP block. These mirror the pre-refactor column names consumed by the
    # LLM prompt builders and evaluation scripts.
    alias_sources: Dict[str, str] = {
        "gene_symbol": "vep_gene_symbol",
        "GeneSymbol": "vep_gene_symbol",
        "mane_select": "vep_mane_select",
        "transcript_id": "vep_transcript_id",
        "protein_id": "vep_protein_id",
        "hgvsc": "vep_hgvsc",
        "hgvsp": "vep_hgvsp",
        "consequence_terms": "vep_consequence_terms",
        "protein_consequence_terms": "vep_consequence_terms",
        "most_severe_consequence": "vep_most_severe_consequence",
        "impact": "vep_impact",
        "variant_allele": "vep_variant_allele",
        "amino_acids": "vep_amino_acids",
        "codons": "vep_codons",
    }

    for alias, source in alias_sources.items():
        if source in kept.columns and alias not in kept.columns:
            kept[alias] = kept[source]
